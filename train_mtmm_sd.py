...
from models.models_MTMM_SD import TSN
...
def visulaize_input(rgb, depth, skeleton):
...
def train(model, train_dataloader, epoch, criterion, optimizer, mse_loss, cos_similar, scaler, model_ema):
    ...
    for step, inputs in enumerate(train_dataloader):
        ...
        if args.modal == 'rgb_depth':
            output, middle_output1, middle_output2, middle_output3, \
                ...
                l_depth_out, g_depth_out = model(rgb)
            # depth: [N, T, 1, 224, 224]
            l_depth_gt = depth.view(
                -1, 1, depth.size(-2), depth.size(-1))
            g_depth_gt = F.interpolate(
                l_depth_gt, size=(56, 56), mode='bilinear')
            g_depth_loss = mse_loss(g_depth_out, g_depth_gt)
            loss = criterion(output, labels) + 0.01 * g_depth_loss
...
def validation(model, val_dataloader, epoch, criterion, is_ema=False):
    ...
    for step, inputs in enumerate(val_dataloader):
        ...
        if args.modal == 'rgb_text':
            output, middle_output1, middle_output2, middle_output3, _, _, _, _, _ = model(
                rgb)
        elif args.modal == 'rgb_depth' or args.modal == 'rgb_skeleton':
            output, middle_output1, middle_output2, middle_output3, _, _, _, _, _, _ = model(
                rgb)
        elif args.modal == 'rgb_depth_text' or args.modal == 'rgb_skeleton_text':
            output, middle_output1, middle_output2, middle_output3, _, _, _, _, _, _, _ = model(
                rgb)
        elif args.modal == 'rgb_depth_skeleton':
            output, middle_output1, middle_output2, middle_output3, _, _, _, _, _, _, _, _ = model(
                rgb)
        elif args.modal == 'rgb_depth_skeleton_text':
            output, middle_output1, middle_output2, middle_output3, _, _, _, _, _, _, _, _, _ = model(
                rgb)
...
def main():
    ...
    cos_similar = nn.CosineSimilarity(dim=1, eps=1e-6).to(device)
    ...
    for epoch in trange(params['epoch_num']):
        train(model, train_dataloader, epoch,
              criterion, optimizer, mse_loss, cos_similar, scaler)
        if epoch % 1 == 0:

...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if self.modal.find('depth') != -1:
            ...
            self.global_decoder = nn.Sequential(
                nn.ConvTranspose2d(2048, 256, kernel_size=4,
                                   stride=2, padding=1),
                nn.BatchNorm2d(256),
                nn.ConvTranspose2d(256, 32, kernel_size=4,
                                   stride=2, padding=1),
                nn.BatchNorm2d(32),
                nn.ConvTranspose2d(32, 1, kernel_size=4,
                                   stride=2, padding=1),
        ...
        if self.modal.find('skeleton') != -1:
            self.local_skel_decoder = nn.Sequential(
                nn.ConvTranspose2d(64, 64, kernel_size=4,
                                   stride=2, padding=1),
                nn.BatchNorm2d(64),
                nn.ConvTranspose2d(64, 42, kernel_size=4,
                                   stride=2, padding=1),
                nn.Sigmoid()
            )
            self.global_skel_decoder = nn.Sequential(
                nn.ConvTranspose2d(2048, 256, kernel_size=4,
                                   stride=2, padding=1),
                nn.BatchNorm2d(256),
        ...
        if self.modal.find('text') != -1:
            self.text_encoder = nn.Sequential(
                nn.Conv1d(2048, 512, kernel_size=self.num_segments),
                nn.BatchNorm1d(512),
            )
    ...
    def forward(self, x):
        ...
        input = x.view((-1, sample_len) + x.size()[-2:])
        #   [NT, C, H, W]
        ...
        # MTMM
        feature_dict = self.feature_extractor(input)

        # Feature Extraction
        local_in = feature_dict['maxpool']
        global_in = feature_dict['layer4']      # [N*S, 2048, 7, 7]
        avgpool_in = feature_dict['avgpool']    # [N*S, 2048, 1, 1]

        # Depth
        if self.modal.find('depth') != -1:
            local_depth_out = self.local_decoder(local_in)
            global_depth_out = self.global_decoder(global_in)

        # Skeleton
        if self.modal.find('skeleton') != -1:
            local_skel_out = self.local_skel_decoder(local_in)
            global_skel_out = self.global_skel_decoder(global_in)

        # Text
        if self.modal.find('text') != -1:
            avgpool_in = avgpool_in.view(-1,
                                         self.num_segments, avgpool_in.size(1))     # [N, S, 2048]
            avgpool_in = avgpool_in.permute(0, 2, 1)    # [N, 2048, S]
            text_out = self.text_encoder(avgpool_in)    # [N, 512, 1]
            text_out = text_out.squeeze(2)              # [N, 512]

        if self.modal == 'rgb':
            return output, middle_output1, middle_output2, middle_output3, final_fea, middle1_fea, middle2_fea, middle3_fea
        elif self.modal == 'rgb_text':
            return output, middle_output1, middle_output2, middle_output3, final_fea, middle1_fea, middle2_fea, middle3_fea, text_out
        elif self.modal == 'rgb_depth':
            return output, middle_output1, middle_output2, middle_output3, final_fea, middle1_fea, middle2_fea, middle3_fea, local_depth_out, global_depth_out
        elif self.modal == 'rgb_skeleton':
            return output, middle_output1, middle_output2, middle_output3, final_fea, middle1_fea, middle2_fea, middle3_fea, local_skel_out, global_skel_out
        elif self.modal == 'rgb_depth_text':
            return output, middle_output1, middle_output2, middle_output3, final_fea, middle1_fea, middle2_fea, middle3_fea, local_depth_out, global_depth_out, text_out
        elif self.modal == 'rgb_skeleton_text':
            return output, middle_output1, middle_output2, middle_output3, final_fea, middle1_fea, middle2_fea, middle3_fea, local_skel_out, global_skel_out, text_out
        elif self.modal == 'rgb_depth_skeleton':
            return output, middle_output1, middle_output2, middle_output3, final_fea, middle1_fea, middle2_fea, middle3_fea, local_depth_out, global_depth_out, local_skel_out, global_skel_out
        elif self.modal == 'rgb_depth_skeleton_text':
            return output, middle_output1, middle_output2, middle_output3, final_fea, middle1_fea, middle2_fea, middle3_fea, local_depth_out, global_depth_out, local_skel_out, global_skel_out, text_out


if __name__ == '__main__':
    net = TSN(83, 8, 'RGB', is_shift=True, base_model='resnet50', shift_div=8,
              img_feature_dim=224, consensus_type='avg', fc_lr5=True, modal='rgb_depth_skeleton_text')
    print(net)
    ...
    output, middle_output1, middle_output2, middle_output3, \
        ...
        local_depth_out, global_depth_out, local_skel_out, global_skel_out, text_out = net(
            input)

    print(output.shape)
    print(middle_output1.shape)
    print(middle_output2.shape)
    print(middle_output3.shape)
    print(final_fea.shape)
    print(middle1_fea.shape)
    print(middle2_fea.shape)
    print(middle3_fea.shape)
    print(local_depth_out.shape)
    print(global_depth_out.shape)

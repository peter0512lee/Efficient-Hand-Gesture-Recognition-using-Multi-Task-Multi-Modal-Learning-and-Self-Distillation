...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            self.base_model = res2net50_26w_4s(
                True if self.pretrain == 'imagenet' else False)
            if self.is_shift:
            ...
            if self.is_shift:
                ...
                for m in self.base_model.modules():
                    if isinstance(m, InvertedResidual) and len(m.conv) == 8 and m.use_res_connect:
                        m.conv[0] = Action(
                            m.conv[0], n_segment=self.num_segments, shift_div=self.shift_div)
            ...
            if self.is_shift:
                ...
                self.base_model = bninception(
                    pretrained=self.pretrain, n_segment=self.num_segments, fold_div=self.shift_div)
    ...
    def forward(self, input, no_reshape=False):
        # pdb.set_trace()
        assert input.size()[
            1] > 3, 'channel and temporal dimension mismatch, tensor size should be: n_batch, n_segment, nc, h, w'
        ...
        if not no_reshape:
            ...
            base_out = self.base_model(input.view(
                (-1, sample_len) + input.size()[-2:]))  # nb*n_segment, c, h, w
        else:
